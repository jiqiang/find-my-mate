...
src/groupView.ts  watches the Group's Members and Positions; hands out the whole map view (§7.4)
src/useGroupView.ts the hook over it — the map renders what it hands back and holds no reading logic
src/camera.ts     the map's first framing and recentre zoom, once, only after the map is ready
src/screens/      FirstRun, Join, Waiting, Map, Members, BlockedPermission
